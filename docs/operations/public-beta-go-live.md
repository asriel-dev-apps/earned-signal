...
```sh
EARNED_SIGNAL_BASE_URL=https://service.example node scripts/beta-smoke.mjs </dev/null
```
...
printf '%s' "$BETA_ACCESS_TOKEN" \
  | EARNED_SIGNAL_BASE_URL=https://service.example \
    EARNED_SIGNAL_TENANT_ID=00000000-0000-4000-8000-000000000001 \
    EARNED_SIGNAL_PROJECT_ID=00000000-0000-4000-8000-000000000002 \
    EARNED_SIGNAL_AUTH_CHECK=1 \
...
`EARNED_SIGNAL_E2E_RUN_ID` はリリースごとに新しくし、同じ実行内の REST/MCP 再送だけで idempotency を確認する。
...
printf '%s\n%s\n%s\n' "$REST_ACCESS_TOKEN" "$MCP_ACCESS_TOKEN" "$RESTRICTED_AGENT_MCP_ACCESS_TOKEN" \
  | EARNED_SIGNAL_BASE_URL=https://service.example \
    EARNED_SIGNAL_E2E_CONFIG=/private/tmp/earned-signal-beta-e2e.json \
    EARNED_SIGNAL_E2E_RUN_ID=release-20260717-1 \
